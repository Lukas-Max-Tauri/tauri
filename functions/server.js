...
import cors from 'cors'; // CORS-Modul importieren
...
const allowedOrigins = process.env.ALLOWED_ORIGINS ? process.env.ALLOWED_ORIGINS.split(',') : [
  ...
  // Neue Firebase Function URL hinzufügen
  ...
  try {
    if (connectionMode === 'local') {
      ...
      if (!mongoURI) {
        ...
        return; // Früher Return statt Fehlerwerfen
      }
    ...
    // Keinen process.exit verwenden und keinen Fehler werfen, nur loggen
  }
};
      ...
      if (!credentialsLoaded) {
        ...
        try {
          credentials = {
            ...
            // Fortführung der private_key-Zeichenkette
...
// Korrigierte globaler Error Handler
app.use((err, req, res, next) => {
  console.error('Global error handler caught:', err);
  res.status(500).json({ 
    error: 'Server error', 
    message: err.message,
    stack: process.env.NODE_ENV === 'production' ? '🥞' : err.stack, // Korrektur hier: err.stack statt error.stack
    mode: connectionMode
  });
});

// Nur den Server starten, wenn nicht als Modul importiert (für lokale Entwicklung oder direkte Ausführung)
...
// Express-App für Firebase Functions exportieren
export default app;
export { app };